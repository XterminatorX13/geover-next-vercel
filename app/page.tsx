...
const HomePage = () => {
  ...
  const MagneticButton = ({
    children,
    mousePosition,
  }: { children: React.ReactNode; mousePosition: { x: number; y: number } }) => {
    const [buttonRef, setButtonRef] = useState<HTMLDivElement | null>(null)
    const [isHovering, setIsHovering] = useState(false)
    const [magneticOffset, setMagneticOffset] = useState({ x: 0, y: 0 })

    useEffect(() => {
      if (!buttonRef || !isHovering) {
      ...
      const distance = Math.sqrt(
        Math.pow(mousePosition.x - buttonCenterX, 2) + Math.pow(mousePosition.y - buttonCenterY, 2),
      )

      if (distance < 50) {
        const offsetX = (mousePosition.x - buttonCenterX) * 0.3
        const offsetY = (mousePosition.y - buttonCenterY) * 0.3
        setMagneticOffset({ x: offsetX, y: offsetY })
      } else {
        setMagneticOffset({ x: 0, y: 0 })
      }
    }, [mousePosition, buttonRef, isHovering])
  ...
  return (
    <div className="min-h-screen bg-background text-foreground relative overflow-hidden">
      ...
      {/* Aurora Background - Only in Dark Mode */}
      <AuroraBackground />
